...
baseline = read.csv("./data_mci/MCI_baseline.csv", skip = 1, na.strings = ".") |> ## import dataset, skip the first row of labeling, set "." as missing value
  janitor::clean_names()|> ## clean and standardize the column names
  mutate(
    sex = 
      case_match(
        sex, 
        1 ~ "male", 
        0 ~ "female"), ## change sex = 1 as male and sex = 0 as female
    sex = as.factor(sex), ## change sex from a numeric to factor variable
    apoe4 = 
      case_match(
        apoe4, 
        1 ~ "APOE4 carrier",  ## change apoe4 = 1 as APOE4 carrier
        0 ~ "APOE4 non-carrier"), ## change apoe4 = 1 as APOE4 non-carrier
    apoe4 = as.factor(apoe4)) |> ## change apoe4 from a numeric to factor variable
  drop_na(age_at_onset) ## remove any participants with no MCI at baseline
```
